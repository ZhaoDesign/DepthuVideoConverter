...
MODEL_DISPLAY_LABELS = {
    ...
    "Base (balanced, ~372 MB)": "Base（均衡，约 372 MB）",
    "Large (best quality, ~1.2 GB)": "Large（高质量，约 1.2 GB）",
    "Small (fastest, ~99 MB)": "Small（最快，约 99 MB）",
    "Base (balanced, ~392 MB)": "Base（均衡，约 392 MB）",
    "Large (best quality, ~1.3 GB)": "Large（高质量，约 1.3 GB）",
}

RESOLUTION_DISPLAY_LABELS = {
    "Original": "原始分辨率",
    "480p": "480p（按原比例，高度 480）",
    "720p": "720p（按原比例，高度 720）",
    "1080p": "1080p（按原比例，高度 1080）",
    "480p (854×480)": "480p（按原比例，高度 480）",
    "720p (1280×720)": "720p（按原比例，高度 720）",
    "1080p (1920×1080)": "1080p（按原比例，高度 1080）",
}
...
def _configure_runtime() -> Path:
    ...
    if project_models_dir.is_dir() and any(project_models_dir.glob("*.pth")):
        default_models_dir = project_models_dir
    else:
        default_models_dir = models_dir
    os.environ.setdefault("DEPTH_MODELS_DIR", str(default_models_dir))
    # The native desktop keeps the local PyTorch path; the upstream ONNX
    # backend remains available to other entry points and packaging scripts.
...
def _unique_output_path(input_path: str, output_dir: str) -> Path:
    ...
    suffix = ".png" if source.suffix.lower() in IMAGE_EXTENSIONS else ".mp4"
    base = target_dir / f"{source.stem}_depth{suffix}"
    if not base.exists():
        return base
    stamp = time.strftime("%Y%m%d-%H%M%S")
    return target_dir / f"{source.stem}_depth_{stamp}{suffix}"
...
class MouseFocusClearingButton(QPushButton):
    """Keep keyboard focus support without leaving a Windows dotted mouse ring."""
...
class FigmaComboBox(QComboBox):
    """Combo box whose popup opens in the available direction without clipping."""
    ...
    def _position_popup(self) -> None:
        ...
        row_height = max(36, view.sizeHintForRow(0) if self.count() else 36)
        row_spacing = max(0, view.spacing())
        visible_count = min(self.count(), self.maxVisibleItems())
        popup_height = 8 + visible_count * row_height + max(0, visible_count - 1) * row_spacing
        popup_width = self.width()

        combo_top_left = self.mapToGlobal(QPoint(0, 0))
        below_y = combo_top_left.y() + self.height()
        above_y = combo_top_left.y() - popup_height
        screen = QApplication.screenAt(combo_top_left) or QApplication.primaryScreen()
        available = screen.availableGeometry() if screen is not None else QRect(0, 0, 10000, 10000)
        ...
        if below_y + popup_height <= available.bottom():
            ...
            popup_y = max(available.top(), below_y)
            popup_height = max(row_height + 8, min(popup_height, available.bottom() - popup_y))

        popup.setGeometry(combo_top_left.x(), popup_y, popup_width, popup_height)
...
class TopBar(QFrame):
    ...
    def _model_menu(self) -> FigmaPopupMenu:
        ...
        for name, cfg in MODEL_DEFS.items():
            ...
            status = "已下载" if Path(cfg["path"]).is_file() else "首次使用时下载"
            menu.add_action_item(f"{model_name} · {status}", lambda: None, active=False)
        menu.add_separator()
        menu.add_action_item("打开模型目录", self._window._open_model_dir, active=True)
        return menu
...
class VideoPlayer(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._clear_btn = MouseFocusClearingButton("×", self._video_box)
        self._clear_btn.setObjectName("videoCloseButton")
        self._clear_btn.setFixedSize(30, 30)
        self._clear_btn.setToolTip("关闭当前视频")
        self._clear_btn.clicked.connect(self.clear)
        self._clear_btn.hide()

        self._fullscreen_btn = MouseFocusClearingButton(self._video_box)
        self._fullscreen_btn.setObjectName("fullscreenButton")
        self._fullscreen_btn.setFixedSize(32, 32)
        self._fullscreen_btn.setToolTip("在软件窗口内放大")
        _set_icon(self._fullscreen_btn, "icon-fullscreen.png", 18)
        ...
        self._video_stack.currentChanged.connect(
            lambda _index: QTimer.singleShot(0, self._refresh_video_masks)
        )
        self._player.mediaStatusChanged.connect(
            lambda _status: QTimer.singleShot(0, self._refresh_video_masks)
        )
    ...
    def load(self, path: str) -> None:
        ...
        self._fullscreen_btn.setVisible(not self._media_is_image)
    ...
    def _toggle_play(self) -> None:
        if self._media_is_image or self._player.source().isEmpty():
            return
...
class DropPanel(QFrame):
    ...
    def __init__(self) -> None:
        ...
        subtitle = QLabel("支持视频与图片：MP4 / MOV / MKV / WEBM / PNG / JPG / WEBP")
        subtitle.setObjectName("muted")
        subtitle.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.choose_button = MouseFocusClearingButton("选择视频或图片")
    ...
    def set_file(self, path: str | None) -> None:
        self.choose_button.setText(Path(path).name if path else "选择视频或图片")
...
class ContourControlWindow(QMainWindow):
    ...
    def _build_menubar(self) -> None:
        ...
        for name, cfg in MODEL_DEFS.items():
            model_name = name.split(" (")[0]
            status = "已下载" if Path(cfg["path"]).is_file() else "首次使用时下载"
            model_act = QAction(f"{model_name} · {status}", self)
            model_act.setEnabled(False)
            model_menu.addAction(model_act)
        model_menu.addSeparator()
        model_dir_act = QAction("打开模型目录", self)
        model_dir_act.triggered.connect(self._open_model_dir)
        model_menu.addAction(model_dir_act)
    ...
    def _change_model_dir(self) -> None:
        ...
        if d:
            ...
            selected_dir = Path(d).expanduser().resolve()
            _m.clear_model_cache()
            _m.MODELS_DIR = selected_dir
            MODELS_DIR = selected_dir
            for cfg in MODEL_DEFS.values():
                cfg["path"] = selected_dir / Path(cfg["path"]).name
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        for key in RESOLUTION_PRESETS:
            self.resolution_combo.addItem(RESOLUTION_DISPLAY_LABELS.get(key, key), key)
    ...
    def _choose_input(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            ...
            "选择视频或图片",
            str(Path.home()),
            "Media files (*.mp4 *.mov *.m4v *.avi *.mkv *.webm *.png *.jpg *.jpeg *.webp *.bmp *.tif *.tiff);;All files (*.*)",
        )
        if path:
    ...
    def _set_input_path(self, path: str) -> None:
        ...
        self.input_kind = "image" if Path(path).suffix.lower() in IMAGE_EXTENSIONS else "video"
        ...
        self.state_label.setText("已选择图片" if self.input_kind == "image" else "已选择视频")
        self.preserve_audio_check.setEnabled(self.input_kind == "video")
    ...
    def _clear_input(self) -> None:
        ...
        self.input_media_stack.setCurrentWidget(self._empty_input_media)
        self.output_dir_label.setText("输出到： 跟随输入视频")
        self.output_dir_label.setToolTip("")
        self.progress_bar.setValue(0)
        self.progress_label.setText("选择一个视频或图片后开始转换。")
        self.state_label.setText("等待视频")
        self.result_label.setText("尚未生成输出视频")
    ...
    def _choose_output_dir(self) -> None:
        start_dir = self.output_dir or str(_default_output_dir(self.input_path))
    ...
    def _start_conversion(self) -> None:
        ...
        output_dir = self.output_dir or str(_default_output_dir(self.input_path))
        self.output_path = None
        self.open_output_btn.setEnabled(False)
        self.open_folder_btn.setEnabled(False)
    ...
    def _set_controls_enabled(self, enabled: bool) -> None:
        ...
        self.preserve_audio_check.setEnabled(enabled and self.input_kind != "image")
...
def _ui_font_family() -> str:
    preferred = "PingFang SC" if sys.platform == "darwin" else "Microsoft YaHei UI"
    if preferred in QFontDatabase.families():
        return preferred

    if os.name == "nt":
        windows_fonts = Path(os.environ.get("WINDIR", r"C:\Windows")) / "Fonts"
        for filename in ("MicrosoftYaHei.ttf", "Noto Sans SC (TrueType).otf", "SourceHanSansCN-Regular.otf"):
...
def apply_style(app: QApplication) -> None:
    ...
    style_sheet = style_sheet.replace(
        '"PingFang SC", "Microsoft YaHei UI", "Segoe UI"',
        f'"{base_font}", "Segoe UI"',
    ).replace(
        '"PingFang SC", "Microsoft YaHei UI", Consolas',
        f'"{base_font}", Consolas',
    )
